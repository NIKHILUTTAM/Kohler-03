...
from kohler_agent.retrieval import SemanticIndex
...
def test_summary_retrieval_can_focus_on_uploaded_source():
    ...
    class FakeDocument:
        def __init__(self, source):
            ...
            self.metadata = {"source": source, "chunk": 1}

    index = SemanticIndex(
        FakeEmbeddingModel(),
        [FakeDocument("old.md"), FakeDocument("uploaded.pdf")],
    )
    selected = index.documents_for_sources(["uploaded.pdf"])
    assert len(selected) == 1
    assert selected[0].metadata["source"] == "uploaded.pdf"
    matches = index.search("uploaded question", sources=["uploaded.pdf"])
    assert matches[0][0].metadata["source"] == "uploaded.pdf"
