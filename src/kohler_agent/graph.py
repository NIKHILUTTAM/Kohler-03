...
def build_graph() -> tuple[Any, dict[str, Any]]:
    ...
    def retrieve_node(state: AgentState) -> AgentState:
        ...
        is_summary_request = bool(
            re.search(
                r"\b(summar(?:y|ize|ise)|overview|key points|main points|"
                r"what is this document|walk me through)\b",
                question,
                flags=re.I,
            )
        )
        if is_summary_request:
            summary_documents = index.documents_for_sources(focus_sources)
            matches = [(document, 1.0) for document in summary_documents]
        else:
            # Do not embed the entire previous conversation for every query.
            # Old assistant text can drown out the current question. Only add
            # the previous turn when the user is clearly referring back to it.
            prior = state.get("history", [])[-2:]
            is_follow_up = bool(
                re.search(
            ...
            matches = index.search(retrieval_query, sources=focus_sources or None)
        sources: list[str] = []
        blocks: list[str] = []
...
When the user asks for a summary or overview, summarize the supplied local
knowledge context directly. Do not refuse merely because the request is short
or uses a pronoun such as "it".
