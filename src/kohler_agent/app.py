    ...
    with st.sidebar:
        ...
        if uploaded is not None:
            ...
            signature = hashlib.sha256(
                uploaded.name.encode("utf-8") + b"\0" + content
            ).hexdigest()
            if st.session_state.get("last_upload_signature") != signature:
                ...
                st.session_state.focus_sources = [destination.name]
                st.cache_resource.clear()
                st.session_state.upload_notice = (
                    f"Saved {destination.name}; rebuilding the knowledge index."
                )
                st.rerun()
                ...
                result = graph.invoke(
                    {
                        ...
                        "focus_sources": st.session_state.get("focus_sources", []),
                    }
                )
