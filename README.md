...
1. **retrieve** — embeds the current question with a local embedding model,
   ranks knowledge-base chunks by cosine similarity, and keeps only chunks
   above a relevance threshold. Recently uploaded files are preferred so a
   question asked immediately after upload is answered from that document.
   Summary requests such as "summarize it" directly read the focused indexed
   chunks because a pronoun-only query has too little vocabulary for semantic
   search.
...
├── tests/test_core.py       # pytest suite (12 tests, same coverage as above)
...
4. Index everything and open the chat UI, where you can:
   ...
   - Upload your own `.txt/.md/.csv/.json/.yaml/.pdf/.docx` documents. The app
     immediately rebuilds the index and focuses the next questions on the
     uploaded file.
   - Ask `summarize it` or `give me an overview` to summarize the most recently
     uploaded document without relying on weak pronoun-only embedding matches.
...
| `KOHLER_SUMMARY_MAX_CHUNKS` | `24` | Maximum chunks used for a direct summary |
